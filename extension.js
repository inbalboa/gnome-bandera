    ...
    class BanderaIndicator extends PanelMenu.Button {
        _init(extension) {
            ...
            if (!this._inputSourceManager) {
                console.error('Bandera: Input source manager not available');
                Main.notify('Bandera', _('Input source manager not available.'));
                this.destroy();
                return;
            }
        ...
        _refillLayoutItems() {
            ...
            this._layoutItems = Array.apply(null, Array(sources.length)).map(function () {});
            for (const source of sources) {
        ...
        _switchSystemIndicator() {
            if (!this._settings) {
                console.error('Bandera: settings not available');
                return;
            }
        ...
        _showKeyboardLayout() {
            try {
                ...
                console.error('Bandera: could not open keyboard layout viewer:', e);
        ...
        _openKeyboardSettings() {
            try {
                ...
                console.error('Bandera: could not open keyboard settings:', e);
